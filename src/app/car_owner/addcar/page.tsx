...
const AddCar = () => {
  ...
  const [termOfUse, setTermOfUse] = useState<TermOfUseResponse>({
    noSmoking: false,
    noPet: false,
    noFoodInCar: false,
    other: false,
  ...
  const convertTermOfUseToString = (termOfUse: TermOfUseResponse): string => {
  ...
  const [additionalFunctions, setAdditionalFunctions] =
    useState<AdditionalFunctionResponse>({
      bluetooth: false,
      gps: false,
      camera: false,
      sunroof: false,
  ...
  const convertAdditionalFuncitonToString = (
    additionalFunctions: AdditionalFunctionResponse
  ): string => {
  ...
  const handleAdditionalFunctionsChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const { name, checked } = event.target;
  ...
  const addTerm = async (idcar: number) => {
    try {
  ...
  const addAddtional = async (idcar: number) => {
    console.log(idcar);
    try {
      const namefunctions =
